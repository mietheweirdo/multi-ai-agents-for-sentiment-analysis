...
try:
    ...
    logger.warning("YouTube Data API not available. Install with: pip install google-api-python-client")
...
class ScrapingConfig:
    ...
    youtube_api_key: Optional[str] = None  # <-- Added for YouTube Data API
    youtube_max_videos: int = 5            # <-- Added for YouTube Data API
    youtube_max_comments: int = 30         # <-- Added for YouTube Data API
...
class YouTubeScraper:
    ...
    def _get_video_comments(self, video_id: str, video_title: str, max_comments: int = 40) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            while comment_count < max_comments:
                ...
                for item in response['items']:
                    ...
                    if content.strip():
                        # Standardize format for pipeline compatibility
                        comments.append({
                            'id': f"{video_id}_{comment_count}",
    ...
    def _search_videos(self, keyword: str, max_results: int = None) -> List[Dict[str, Any]]:
        """Search YouTube videos and return top results"""
        if max_results is None:
            max_results = self.config.youtube_max_videos
    ...
    def _get_video_comments(self, video_id: str, video_title: str, max_comments: int = None) -> List[Dict[str, Any]]:
        """Fetch comments for a single video"""
        ...
        try:
            ...
            logger.error(f"Unexpected error getting comments for {video_id}: {e}")
        
        return comments

    def scrape_by_keyword(self, keyword: str) -> List[Dict[str, Any]]:
        """Scrape YouTube videos by keyword using YouTube Data API"""
        if not self.config.youtube_api_key:
            logger.warning("YouTube API key not configured - skipping YouTube keyword search")
            logger.info("Set youtube_api_key in ScrapingConfig to enable YouTube search")
            return []
        
        if not YOUTUBE_API_AVAILABLE:
            logger.warning("Google API client not installed - skipping YouTube keyword search")
            logger.info("Run: pip install google-api-python-client")
            return []
        ...
        logger.info(f"Found {len(videos)} videos, getting comments...")
        ...
        logger.info(f"Total YouTube comments collected: {len(all_comments)}")
        return all_comments
...
class AdvancedPreprocessor:
    ...
    def _is_quality_text(self, text: str) -> bool:
        ...
        # Length checks
        if len(text) < self.config.min_content_length or len(text) > self.config.max_content_length:
            return False
...
def scrape_and_preprocess(keyword: str = None, youtube_urls: List[str] = None, 
                         ...
                         sources: List[str] = None, max_items_per_source: int = None,
                         pipeline: 'IntegratedDataPipeline' = None) -> List[Dict[str, Any]]:
    """
    ...
    Args:
        ...
        pipeline: Optional pipeline instance to use
        
    Returns:
        List of agent-ready data items
    """
    if pipeline is None:
        # Try to load config.json for YouTube API key if available
            ...
            if os.path.exists(config_path):
                with open(config_path, 'r') as f:
                    ...
                    scraping_config = ScrapingConfig(
                        ...
                        youtube_max_videos=config_data.get('youtube_max_videos', 5),
                        youtube_max_comments=config_data.get('youtube_max_comments', 30)
                    )
                    logger.info("Loaded YouTube API configuration from config.json")
        except Exception as e:
    ...
    # Update pipeline config if max_items_per_source is specified
    if max_items_per_source is not None:
        pipeline.scraping_config.tiki_max_reviews_per_product = max_items_per_source
